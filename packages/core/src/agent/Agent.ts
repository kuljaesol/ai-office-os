...
export class Agent {
    ...
    async think(perception: Perception): Promise<Decision> {
        ...
        try {
            ...
            const toolList = (this.config.capabilities || []).map((c: any) => c.name).join(', ') || 'none';
...
You must decide your next action. Output ONLY one raw JSON object with no markdown, no code fences, and no text before or after it. Format:
{
...
- You may ONLY use these exact tool names for toolCall.name: ${toolList}. NEVER invent any other tool name. If no tool fits, use action "talk" or "idle".`;
            ...
            const raw = (res.content || '').replace(/```json/gi, '').replace(/```/g, '');
            const match = raw.match(/\{[\s\S]*\}/);
            const decisionStr = match ? match[0] : null;
            if (!decisionStr) {
                console.warn(`[Agent ${this.config.name}] No JSON, speaking raw reply.`);
                if (raw.trim()) {
                    return { thought: raw.slice(0, 120), action: 'talk', target: 'User', message: raw.slice(0, 240) };
                }
            }
            ...
            if (decisionStr) {
                let parsed: any;
                try {
                    parsed = JSON.parse(decisionStr);
                } catch {
                    console.warn(`[Agent ${this.config.name}] JSON parse failed, speaking raw.`);
                    return { thought: raw.slice(0, 120), action: 'talk', target: 'User', message: raw.slice(0, 240) };
                }
