...
const FURNITURE_PALETTE: { type: FurnitureItem['type']; emoji: string; label: string }[] = [
    { type: 'desk', emoji: '🖥️', label: 'โต๊ะทำงาน' },
    { type: 'plant', emoji: '🌿', label: 'ต้นไม้' },
    { type: 'bookshelf', emoji: '📚', label: 'ชั้นหนังสือ' },
    { type: 'coffee_machine', emoji: '☕', label: 'กาแฟ' },
    { type: 'table', emoji: '🪑', label: 'โต๊ะ' },
    { type: 'chair', emoji: '💺', label: 'เก้าอี้' },
    { type: 'whiteboard', emoji: '📝', label: 'ไวต์บอร์ด' },
];
    ...
    return (
        <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <h3 style={{ margin: 0, fontSize: '14px' }}>🏗️ แก้ผังออฟฟิศ</h3>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '11px', marginBottom: 8, color: '#c9cff8' }}>
                ...
                ลากวางเฟอร์นิเจอร์ด้วยเมาส์บนจอ
            </label>
            ...
            <button onClick={addItem} style={{
                ...
                + เพิ่ม {FURNITURE_PALETTE.find(f => f.type === selected)?.label}
            </button>
            ...
            <div style={{ flex: 1, overflowY: 'auto', fontSize: '11px', marginBottom: 8 }}>
                {items.length === 0 && (
                    <p style={{ color: '#555', fontStyle: 'italic', margin: 0 }}>
                        เลือกชนิดเฟอร์นิเจอร์แล้วกดเพิ่มเพื่อวาง
                    </p>
                )}
                {items.map(item => (
