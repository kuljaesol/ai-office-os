...
export function AgentPulseBoard() {
    ...
    return (
        <FloatingPanel
            id="agent-pulse"
            title="ชีพจร Agent"
            subtitle="อารมณ์ ความเสี่ยง โมเมนตัม"
            width={300}
            defaultDock="left"
            defaultY={420}
            zIndex={14}
        >
            {sortedAgents.length === 0 && (
                <div style={{ fontSize: 11, color: '#86c6b5' }}>กำลังรอข้อมูลสด...</div>
            )}
            {sortedAgents.map((agent) => (
                <div key={agent.id} style={{
                    ...
                    <div style={{ fontSize: 12, fontWeight: 700 }}>
                        {agent.name} <span style={{ fontWeight: 400, color: '#98ccb9' }}>({ACTION_TH[agent.action] || agent.action})</span>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 10, marginTop: 4, color: '#cbe9df' }}>
                        <div>อารมณ์: {pct(agent.mood)}</div>
                        <div>ชื่อเสียง: {pct(agent.reputation)}</div>
                        <div>ความเสี่ยง: {pct(agent.riskLevel)}</div>
                        <div>โมเมนตัม: {pct(agent.momentum)}</div>
                    </div>
                </div>
            ))}
        </FloatingPanel>
