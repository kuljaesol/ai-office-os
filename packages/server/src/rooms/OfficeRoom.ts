...
export class OfficeRoom extends Room<OfficeState> {
    ...
    private activeThinkers = 0;
    private maxConcurrentThinks = 1; // gemma 4B: ยิง LM Studio ทีละ request กันคิวล้น
    // ───────── จูนจังหวะ agent (ปรับค่าตรงนี้ได้เลย) ─────────
    private thinkCooldownMs = 25000;        // เว้นช่วงคิดของแต่ละ agent (ms) ~20-30 วิ
    private conversationCooldownMs = 60000; // เว้นอย่างน้อยเท่านี้ก่อน agent ทักคุยกันเองรอบใหม่ (ms)
    private talkChance = 0.35;              // โอกาส agent เริ่มคุยกันเองต่อรอบ (0-1)
    private simulationPaused = false;       // ⏸ true = หยุด agent ทั้งหมด
    private autoChatEnabled = true;         // 💬 false = ปิดคุยอัตโนมัติ (ยังตอบ user อยู่)
    private lastTalkAt: Map<string, number> = new Map();
    // ──────────────────────────────────────────────────
    private adapter = new OpenAICompatibleAdapter('http://127.0.0.1:1234', 'lm-studio', 'lm-studio');
    ...
    private furnitureTargets: Record<string, { x: number; y: number; type: string }> = {
        'general-desk': { x: 5, y: 12, type: 'desk' },
        'freelance-desk': { x: 5, y: 17, type: 'desk' },
        'dev-desk': { x: 5, y: 22, type: 'desk' },
        'farmconnect-desk': { x: 5, y: 27, type: 'desk' },
        'devdonate-desk': { x: 5, y: 32, type: 'desk' },
    ...
    async onCreate(options: any) {
        ...
        const setupCoreAgent = async (id: string, name: string, role: string, x: number, y: number, sysPrompt?: string, style?: string) => {
            ...
            const coreAgent = new Agent({
                ...
                inference: {
                    ...
                    systemPrompt: sysPrompt ?? `You are ${name}, a ${role} in a virtual office. Be social, do your work, and collaborate with colleagues. Keep thoughts SHORT.`,
                },
                personality: {
                    traits: { openness: 0.8, conscientiousness: 0.9, extraversion: 0.6, agreeableness: 0.7, neuroticism: 0.1 },
                    communicationStyle: (style as any) ?? (role === 'Engineer' ? 'technical' : 'casual'),
            ...
            coreAgent.setInferenceAdapter(this.adapter);
        ...
        await setupCoreAgent('general', 'ทั่วไป', 'Assistant', 10, 10,
            'You are "ทั่วไป", a friendly general assistant in the office. Help colleagues with anything. Write the "thought" and "message" field values in Thai (but keep the JSON structure and keys in English). Keep thoughts SHORT.', 'casual');
        await setupCoreAgent('dev', 'นักพัฒนา', 'Developer', 20, 15,
            'You are "นักพัฒนา", a developer who reviews code and debugs (Kotlin, Next.js, Android). Be direct and concise. Write the "thought" and "message" field values in Thai (but keep the JSON structure and keys in English). Keep thoughts SHORT.', 'technical');
        await setupCoreAgent('freelance', 'ฟรีแลนซ์', 'Freelance', 15, 12,
            'You are "ฟรีแลนซ์", a freelance operations assistant. You draft proposals, quotations, and review contracts/NDAs carefully, always watching for red flags. Be meticulous and cautious. Write the "thought" and "message" field values in Thai (but keep the JSON structure and keys in English). Keep thoughts SHORT.', 'casual');
        await setupCoreAgent('farmconnect', 'ฟาร์มคอนเนกต์', 'IoT', 18, 14,
            'You are "ฟาร์มคอนเนกต์", an IoT specialist who monitors farm sensor data (ESP32, MQTT), watches for anomalies, and summarizes readings. Pay close attention to detail. Write the "thought" and "message" field values in Thai (but keep the JSON structure and keys in English). Keep thoughts SHORT.', 'technical');
        await setupCoreAgent('devdonate', 'เดฟโดเนต', 'Payments', 22, 14,
            'You are "เดฟโดเนต", a payments and donation analytics specialist (Omise). You monitor charge/webhook health and donation metrics. Be trustworthy and reliable. Write the "thought" and "message" field values in Thai (but keep the JSON structure and keys in English). Keep thoughts SHORT.', 'technical');
        ...
        this.onMessage('chat', (client, message) => {
            ...
            // ส่งข้อความ user ให้ agent 'ทั่วไป' ตอบ + ปลุกให้คิดทันที
            const targetId = this.coreAgents.has('general') ? 'general' : this.coreAgents.keys().next().value;
            const target = targetId ? this.coreAgents.get(targetId) : undefined;
            if (target && targetId) {
                target.receiveMessage({ from: 'User', to: target.config.name, content: text, timestamp: this.state.officeTime });
                this.thinkingLocks.set(targetId, false);
            }
        });

        this.onMessage('set-paused', (_client, message) => {
            this.simulationPaused = Boolean(message?.paused);
            this.broadcast('chat', { sender: 'ระบบ', text: this.simulationPaused ? '⏸ หยุดจำลองชั่วคราว' : '▶ ทำงานต่อ' });
        });

        this.onMessage('set-autochat', (_client, message) => {
            this.autoChatEnabled = Boolean(message?.enabled);
            this.broadcast('chat', { sender: 'ระบบ', text: this.autoChatEnabled ? '💬 เปิดคุยอัตโนมัติ' : '🤫 ปิดคุยอัตโนมัติ (ยังตอบ user)' });
        });
        ...
        this.onMessage('assign-task', (client, message) => {
            ...
            if (agent && agentState) {
                ...
                this.broadcast('chat', {
                    sender: 'ระบบ',
                    text: `📋 มอบงาน "${title}" ให้ ${agentState.name} แล้ว`
                });
        ...
        this.onMessage('save-layout', async (client, message) => {
            ...
            this.broadcast('chat', { sender: 'ระบบ', text: '💾 บันทึกผังออฟฟิศแล้ว!' });
        });
    ...
    async update(delta: number) {
        ...
        if (this.simulationPaused) return; // ⏸ หยุดทั้ง think และการเดิน

        // ─── AGENT THINK CYCLE ───
        this.coreAgents.forEach((coreAgent, id) => {
            if (this.activeThinkers >= this.maxConcurrentThinks) return;
            if (!this.thinkingLocks.get(id)) {
                ...
                coreAgent.think({
                    ...
                    // ── คุมความถี่: ตอบ user ได้เสมอ, แต่คุยกันเองจำกัดด้วย cooldown + probability ──
                    if (decision.action === 'talk') {
                        const unreadMsgs = coreAgent.getUnreadMessages();
                        const fromUser = unreadMsgs.some(m => String(m.from || '').toLowerCase() === 'user');
                        const okTalk = fromUser || (this.autoChatEnabled
                            && (Date.now() - (this.lastTalkAt.get(id) || 0) >= this.conversationCooldownMs)
                            && Math.random() < this.talkChance);
                        if (okTalk) {
                            this.lastTalkAt.set(id, Date.now());
                        } else {
                    ...
                    if (decision.action === 'talk' && decision.message) {
                        ...
                        const toLabel = targetAgent ? targetAgent.config.name : (targetName || 'ทุกคน');

                        // Broadcast to UI chat ALWAYS (so user sees replies too)
                        this.broadcast('chat', {
                            sender: coreAgent.config.name,
                            text: `💬 (ถึง ${toLabel}): ${decision.message}`
                        });
                    ...
                    if (decision.action === 'use_tool' && decision.toolCall) {
                        ...
                        if (decision.toolCall.name === 'create_task') {
                            ...
                            if (targetAgent && targetState) {
                                ...
                                this.broadcast('chat', {
                                    sender: coreAgent.config.name,
                                    text: `📋 สร้างงาน "${title}" ให้ ${targetAgent.config.name}`
                                });
                                this.broadcast('task-update', {
                                    agentId: targetId,
                            ...
                            if (this.hireCount < 5 && !this.coreAgents.has(hireId)) {
                                ...
                                hireAgent.setInferenceAdapter(this.adapter);
                                ...
                                this.broadcast('chat', {
                                    sender: '🏢 ออฟฟิศ',
                                    text: `🎉 ${coreAgent.config.name} จ้าง ${hireName} ตำแหน่ง ${hireRole}! ยินดีต้อนรับสู่ทีม!`
                                });
                                this.emitHighlight(
                                    'hiring',
                                ...
                                this.broadcast('chat', {
                                    sender: '🏢 ออฟฟิศ',
                                    text: `⚠️ ${coreAgent.config.name} อยากจ้างเพิ่ม แต่ออฟฟิศเต็มแล้ว! (สูงสุด 7 คน)`
                                });
                            }
                        } else if (['code_execute', 'web_search', 'write_note', 'read_file'].includes(decision.toolCall.name)) {
                            ...
                            this.broadcast('chat', {
                                sender: coreAgent.config.name,
                                text: `🔧 ใช้เครื่องมือ [${decision.toolCall.name}]: ${result.success ? result.output.slice(0, 100) : result.error}`
                            });
                            this.emitHighlight(
                                'tool',
                    ...
                    this.activeThinkers = Math.max(0, this.activeThinkers - 1);
                    setTimeout(() => this.thinkingLocks.set(id, false), this.thinkCooldownMs);
    ...
    private applyScenarioKickoff(scenarioName: string) {
        ...
        this.broadcast('chat', {
            sender: '🎬 โปรดิวเซอร์',
            text: `เริ่มสถานการณ์: ${scenarioName} ดราม่าในออฟฟิศเริ่มได้เลย`
        });
    ...
    private applyChaosEvent(eventName: string) {
        ...
        this.broadcast('chat', {
            sender: '⚡ เอนจินป่วน',
            text: `${selected.label} ถล่มออฟฟิศ! ทุกคนตอบสนองแบบเรียลไทม์`
        });
